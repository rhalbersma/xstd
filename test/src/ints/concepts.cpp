...
// Cv-qualification is transparent to all six, so the lattice is the same one qualified.
BOOST_AUTO_TEST_CASE(TheRefinementsSurviveCvQualification)
{
        ...
        static_assert(xstd::alignable<unsigned const> == xstd::alignable<unsigned>);
        BOOST_CHECK(true);
}
