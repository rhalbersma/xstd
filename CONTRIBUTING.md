...
- **Every required compiler/platform leg passes.** See the table in [README.md](README.md) for the current matrix (GCC, Clang, Clang-CL, MSVC, MinGW, AppleClang). The GCC, Clang, and MinGW `Trunk / Preview` legs are required, not advisory: xstd tracks trunk deliberately. Only the MSVC `2026-Preview` leg and the Clang-CL legs remain best-effort, for reasons specific to those toolchains (see the workflow files).
