...
| [Abseil](https://abseil.io/) | `test/src/abseil.cpp` only | The one test dependency that is a compiled library rather than headers (`absl::int128`). Declared in the same manifest; set `XSTD_TEST_ABSEIL=OFF` to drop the test and the dependency together |
