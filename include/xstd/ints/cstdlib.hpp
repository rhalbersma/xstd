//          Copyright Rein Halbersma 2014-2026.
// Distributed under the Boost Software License, Version 1.0.
//    (See accompanying file LICENSE_1_0.txt or copy at
//          http://www.boost.org/LICENSE_1_0.txt)

#ifndef XSTD_CSTDLIB_HPP
#define XSTD_CSTDLIB_HPP

// Types
#include <xstd/ints/cstdlib/div_result.hpp> // IWYU pragma: export; div_result

// Miscellaneous algorithms and math
#include <xstd/ints/cstdlib/sign.hpp>         // IWYU pragma: export; sign
#include <xstd/ints/cstdlib/abs.hpp>          // IWYU pragma: export; abs
#include <xstd/ints/cstdlib/unsigned_abs.hpp> // IWYU pragma: export; unsigned_abs
#include <xstd/ints/cstdlib/div.hpp>          // IWYU pragma: export; div
#include <xstd/ints/cstdlib/div_euclid.hpp>   // IWYU pragma: export; div_euclid
#include <xstd/ints/cstdlib/div_floor.hpp>    // IWYU pragma: export; div_floor

#endif // XSTD_CSTDLIB_HPP
