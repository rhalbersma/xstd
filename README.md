...
| `<xstd/type_traits.hpp>` | `is_integral_like` <br> `is_arithmetic_like` <br> `is_signed_like` <br> `is_unsigned_like` <br> `make_signed_like` <br> `make_unsigned_like` <br> `is_specialization_of` <br> `empty_type` <br> `conditional_data_member_t` | `std::is_integral`, opened to integer-class types <br> `std::is_arithmetic`, opened through the integral half <br> `std::is_signed`, opened the same way <br> `std::is_unsigned`, opened the same way <br> Open, user-specializable `std::make_signed` <br> Open, user-specializable `std::make_unsigned` <br> Is a type a class template specialization? <br> A tagged empty type <br> A conditionally present member | [iterator.concept.winc] (`is-integer-like`) <br> none <br> none <br> none <br> none <br> none <br> [p2098r1](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2020/p2098r1.pdf) (not adopted) <br> none <br> none |
