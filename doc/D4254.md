...
The main motivations for this proposal are:
  ...
  - the suffixes `z` and `t` are consistent with the length modifiers for formatted I/O in the C standard library (see also the section Design Decisions).
  
The scope of this proposal is limited to literal suffixes for the support types `size_t` and `ptrdiff_t` from the Standard Library header `<cstddef>`.

Note that a technically similar proposal could be made for literal suffixes for all the integer types from the Standard Library header `<cstdint>`, such as `uX` for types `uintX_t`, with `X` running over `{ 8, 16, 32, 64 }`. However, this would require a more thorough analysis of a good naming scheme that is both brief, intuitive, and without name clashes with other user-defined literals in the Standard Library. Furthermore, these fixed-size integers arise much less frequently when using the standard containers or algorithms. We therefore do not propose to add literal suffixes for the integer types from `<cstdint>`.   
  
Extended Example
----------------

**1**. As an illustrative example, consider looping over a `vector` and accessing both the loop index `i` as well as the vector elements `v[i]`
...
**2**. Note that the above example is not meant as the definitive style for all index-based `for` loops. E.g., this particular example might be improved by a modified range-based `for` statement that emits a `size_t` index deduced from a zero-based integer range object initialized from `v.size()`

    // not actually proposed here
    for (auto i : std::integral_range(v.size()) { /* ... */ }  

However, for non-zero-based integer ranges (e.g. when skipping the first element), the same type deduction issues would reappear, and it would become convenient to be able to write
