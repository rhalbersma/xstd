...
them and no umbrella gathers them, so an adapted library is asked for by
name and `<xstd/cstdint.hpp>` never drags in Abseil. Each hard-includes the
library it adapts, leaving the `__has_include` probe to the consumer — which
the test suite writes for itself, having no fixture headers for these two and
reaching them through the shipped adapter the way anyone else would.
